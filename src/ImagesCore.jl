...
import Base: convert, copy, copy!, ctranspose, delete!, done, eltype,
             float32, float64, get, getindex, haskey, maximum, minimum,
             next, ndims, parent, permutedims,
             reinterpret, setindex!, show, showcompact, similar, size,
             slice, squeeze, start, strides, sub, writemime
...
# float32, float64, etc. These are still useful here because of Image{RGB{U8}} -> Image{RGB{Float32}} is an annoying level of specification for the user.
...
function _pixelspacing(img::AbstractImage)
    if haskey(img, "spacedirections")
        ...
        return __pixelspacing(sd)
    end
    ones(sdims(img))
end

@noinline __pixelspacing(sd) = [maximum(abs(sd[i])) for i = 1:length(sd)]
