using FactCheck, ImagesCore, Colors, FixedPointNumbers, ColorVectorSpace
...
facts("Core") do
    ...
    img = ImageCmap(copy(B), cmap, Dict{ASCIIString, Any}([("pixelspacing", [2.0, 3.0]), ("spatialorder", ImagesCore.yx)]))
    ...
    context("Indexed color") do
        ...
        local img_ = ImageCmap(copy(B), cmap, Dict{ASCIIString, Any}([("spatialorder", ImagesCore.yx)]))
        @fact colorspace(img_) --> "RGB"
        img_ = ImageCmap(copy(B), cmap, spatialorder=ImagesCore.yx)
        @fact colorspace(img_) --> "RGB"
    ...
    context("Dims and ordering") do
        ...
        @fact properties(tmp) --> Dict{ASCIIString,Any}([("spatialorder",ImagesCore.yx)])
        copy!(tmp, imgd, "spatialorder", "pixelspacing")
        if testing_units
            @fact tmp["pixelspacing"] --> [2.0mm, 3.0mm]
        end

        @fact storageorder(img) --> ImagesCore.yx
        @fact storageorder(imgds) --> [ImagesCore.yx; "color"]
...
# @fact colordim(s) --> 0
# @fact colorspace(s) --> "Unknown"
# @fact spatialorder(s) --> ImagesCore.yx
# @fact s.data --> imgds[:,:,2]
# reslice!(s, sd, 3)
# @fact s.data --> imgds[:,:,3]
...
##        @fact chan --> Blookup
    ...
    context("Spatial order, width/ height, and permutations") do
        @fact spatialpermutation(ImagesCore.yx, imgds) --> [1,2]
        @fact widthheight(imgds) --> (5,3)
        C = convert(Array, imgds)
        @fact C --> imgds.data
        imgds["spatialorder"] = ["x", "y"]
        @fact spatialpermutation(ImagesCore.xy, imgds) --> [1,2]
        @fact widthheight(imgds) --> (3,5)
        C = convert(Array, imgds)
        @fact C --> permutedims(imgds.data, [2,1,3])
        imgds.properties["spatialorder"] = ["y", "x"]
        @fact spatialpermutation(ImagesCore.xy, imgds) --> [2,1]
        imgds.properties["spatialorder"] = ["x", "L"]
        @fact spatialpermutation(ImagesCore.xy, imgds) --> [1,2]
        imgds.properties["spatialorder"] = ["L", "x"]
        @fact spatialpermutation(ImagesCore.xy, imgds) --> [2,1]
        A = randn(3,5,3)
        @fact spatialpermutation(ImagesCore.xy, A) --> [2,1]
        @fact spatialpermutation(ImagesCore.yx, A) --> [1,2]

        imgds.properties["spatialorder"] = ImagesCore.yx
    ...
    context("Reinterpret, separate, more convert") do
        ...
        @fact spatialorder(imrgb8) --> ImagesCore.yx
        ...
        @fact spatialorder(imgray) --> ImagesCore.yx
        ...
        imgs = subim(img, :, :)
        @fact isa(minimum(imgs), Gray{UFixed16}) --> true  # change in behavior from Images issue#232
        # Raw
